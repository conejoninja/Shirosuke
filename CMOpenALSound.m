...
@synthesize error, temporarySounds, duration;
...
- (void) deleteTemporarySource
{
	...
	NSLog(@"Deleting temporary source: sourceID: %u, [temporarySounds count]:%u", tmpSourceID, [temporarySounds count]);	
...
- (void) setPitch:(ALfloat)newPitch
{
	...
	{
		//tmpSourceID is the source ID for the temporary sound
		alSourcef([tmpSourceID unsignedIntegerValue], AL_PITCH, volume);
	}
}
@end
